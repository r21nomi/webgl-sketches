...
vec3 repetition(vec3 ray, float offset) {
    vec3 repeatedRay = fract(ray * offset);
    repeatedRay -= 0.5;
...
float sceneDist(vec3 ray) {
    vec3 repeatedRay1 = repetition(ray, 0.6);
    vec3 repeatedRay2 = repetition(ray, 0.6);

    float t = time * 1.0;
    float step = radians(180.0);
    ...
    float barDistance1 = barDist(repeatedRay1.xz, map(sin(speed), -1.0, 1.0, 0.1, 0.3));
    float barDistance2 = barDist(repeatedRay2.yz, map(sin(speed), -1.0, 1.0, 0.1, 0.3));
...
void main(void) {
    ...
    st *= rotate2d(time * 0.3);

    vec3 cameraPosition = vec3(sin(time),  0.0, -time);
